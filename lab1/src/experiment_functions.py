...
def parameter_testing(solver: gen_sol.GeneticSolver, pop_size: int, file_name: str, values_to_test: np.ndarray[float], parameter_name: str) -> str:
    test_population = gen_sol.create_first_population(pop_size)
    with open(file_name, "w") as file_handler:
        if parameter_name == "mutation probability":
            func = solver.set_mutation_p
        elif parameter_name == "crossover probability":
            func = solver.set_crossover_p
...
if __name__ == '__main__':
    ...
    print("What parameter will you test (mutation probability/crossover probability?: ")
