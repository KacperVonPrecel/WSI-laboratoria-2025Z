...
class SARSA():
    ...
    def learn(self, state, action, reward, next_state, next_action):
        current_q = self.get_q_values(state)[action]
        next_q_value = self.get_q_values(next_state)[next_action]

        new_q = current_q + self.learning_rate * (reward + self.gamma * next_q_value - current_q)
        self.q_table[state][action] = new_q


def train_generic_qlearning(env: gym.Env, agent: QLearningAgent, state_processor_func, episodes=500):
    history = []
...
def train_generic_sarsa(env: gym.Env, agent: SARSA, state_processor_func, episodes=500):
    history = []
